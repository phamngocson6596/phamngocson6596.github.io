...
if (localStorage.localDanhmuc) {
  iDanhmuc = JSON.parse(localStorage.localDanhmuc);
}
        ...
        <td id="motherContent${index + 1}">
            <div class="w3-cell-row" style="display:table">
                ...
                <div class="hiddenButton w3-cell-middle" style="width:5%">
                    <div class="w3-button w3-text-blue editContent w3-cell" id="edit${
                      index + 1
                    }" title="Chỉnh sửa" style="padding:0px;padding-left:5px;"><i class="fas fa-edit"></i></div>  
                    <div class="w3-button w3-text-blue miniReset w3-cell" id="reset${
                      index + 1
                    }" title="Reset hàng" style="padding:0px;padding-left:5px;"><i class="fas fa-redo"></i></div>
                </div>
            </div>
        </td>
        <td class="bc"><label class="custom-radio"><input type="radio" name="radio${
          index + 1
        }" tabindex="-1"><span class="checkmark"></span></label></td>
        <td class="sy"><label class="custom-radio"><input type="radio" name="radio${
          index + 1
        }" tabindex="-1"><span class="checkmark"></span></label></td>
        <td class="bp"><label class="custom-radio"><input type="radio" name="radio${
          index + 1
        }" tabindex="-1"><span class="checkmark"></span></label></td>
...
document.querySelectorAll(".w3-button.editContent").forEach((button) => {
  button.addEventListener("click", () => {
    const mother = document.querySelector(
      `#motherContent${button.id.substring(4)}`
    );
...
document.querySelector(".w3-badge.person").onclick = function () {
  document.getElementById("id02").style.display = "block";
  document.getElementById("id02").querySelector("input").focus();
};
...
document.querySelector(
  "#tablecontent tr:nth-of-type(1) .bc input"
).checked = true;
document.querySelector("#tablecontent tr:nth-of-type(1) textarea").value = "1-";
document.querySelector(
  "#tablecontent tr:nth-of-type(1) .miniontosum"
).value = 1;
document.querySelector(
  "#tablecontent tr:nth-of-type(2) .miniontosum"
).value = 1;
document.querySelector(
  "#tablecontent tr:nth-of-type(2) .bc input"
).checked = true;

if (localStorage.tenthuky) {
  document.querySelector("#tenthuky").querySelector("b").textContent =
    localStorage.tenthuky;
}
...
document.querySelectorAll(".ccv").forEach((ccv) => {
  ccv.addEventListener("click", (event) => {
    document.querySelector(
      "#congchungvien"
    ).innerHTML = `<b>${event.currentTarget.textContent}</b>`;
  });
});
